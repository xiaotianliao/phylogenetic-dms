...
# Usage
input_file <- '/Users/xl7/Documents/0.Projects/02.phylo-dms/00.inputs/lysr_asr_lg/lysr_asr_lg.state'
sequences <- extract_sequences(input_file)
head(sequences)

output_file <- '/Users/xl7/Documents/0.Projects/02.phylo-dms/00.inputs/lysr_asr_lg/lysr_reconstructed_sequences.fasta'
save_sequences(sequences, output_file)


#######################################################################################
# Read the FASTA file
fasta_file <- "/Users/xl7/Documents/0.Projects/02.phylo-dms/00.inputs/lysr_asr_lg/mafft_fft-ns-i_lysr_asr.fasta"
...
output_file <- "/Users/xl7/Documents/0.Projects/02.phylo-dms/02.outputs/lysr_lg_sequence_comparisons.csv"
...
#######################################################################################
# Read the CSV file
data <- read.csv("/Users/xl7/Documents/0.Projects/02.phylo-dms/02.outputs/lysr_lg_sequence_comparisons.csv")
...
write.csv(filtered_data %>% select(transition, mutations, mutation_count), "/Users/xl7/Documents/0.Projects/02.phylo-dms/02.outputs/lysr_lg_filtered_sequence_comparisons.csv", row.names = FALSE)
...
p1 <- ggplot(filtered_data, aes(x = mutation_count)) +
  ...
  ggtitle("LysR (203 sequences) - All Transitions") +  
  xlab("Mutation Count") +  
  ylab("Frequency") +  
  theme_classic() + 
  theme(
...
p2 <- ggplot(filtered_data, aes(x = mutation_count)) +
  geom_histogram(binwidth = 1, fill = viridis(1), color = "black", alpha = 0.1) +
  geom_vline(xintercept = 5, color = "brown3", linetype = "dashed", size = 0.5) +
  annotate("text", x = 2,5, y = 13, label = "14.1%", color = "brown3", size = 4) +
