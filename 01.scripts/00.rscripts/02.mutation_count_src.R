...
#######################################################################################
# Read the FASTA file
fasta_file <- "/Users/xl7/Documents/0.Projects/02.phylo-dms/00.inputs/src_mammalia_asr/src_mafft_einsi_web.fasta"
...
#######################################################################################
# Read the CSV file
data <- read.csv("/Users/xl7/Documents/0.Projects/02.phylo-dms/02.outputs/src_sequence_comparisons.csv")
...
write.csv(filtered_data %>% select(transition, mutations, mutation_count), "/Users/xl7/Documents/0.Projects/02.phylo-dms/02.outputs/src_filtered_sequence_comparisons.csv", row.names = FALSE)
...
p2 <- ggplot(filtered_data, aes(x = mutation_count)) +
  geom_histogram(binwidth = 1, fill = viridis(1), color = "black", alpha = 0.1) +
  geom_vline(xintercept = 5, color = "brown3", linetype = "dashed", size = 0.5) +
  annotate("text", x = 2,5, y = 13, label = "77.8%", color = "brown3", size = 4) +
